...
int unit_walktoxy(struct block_list *bl, short x, short y, int flag)
{
	...
	if (bl->type == BL_PC)
	{
		...
		if (sd && battle_config.cellpvp_walkout_delay > 0 && DIFF_TICK(gettick(), sd->pvpcan_walkout_tick) < battle_config.cellpvp_walkout_delay && !map->getcell( sd->bl.m, x, y, CELL_CHKPVP ) )
		{
			int e_tick = (int)(DIFF_TICK(gettick(), sd->pvpcan_walkout_tick )/1000);
			char e_msg[150];
			e_tick = (battle_config.cellpvp_walkout_delay/1000) - e_tick;
			if( e_tick > 99 )
					sprintf(e_msg, "Aguarde para poder Sair da Área PK. Tempo %.1f minutos.", (double)e_tick / 60);
			else
					sprintf(e_msg, "Aguarde para poder Sair da Área PK. Tempo %d segundos.", e_tick);
