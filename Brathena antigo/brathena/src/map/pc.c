               ...
               if ( !itemdb_isspecial( sd->status.inventory[n].card[0] ) ) {
                       for ( slot = 0; slot < MAX_SLOTS; slot++ ) {
                               if ( sd->status.inventory[n].card[slot] ) {
                                       struct item_data *i_data = itemdb_exists( sd->status.inventory[n].card[slot] );
...
int pc_dead(struct map_session_data *sd,struct block_list *src)
{
	...
	if(battle_config.bone_drop==2 || (battle_config.bone_drop==1 && (map->list[sd->bl.m].flag.pvp || (map->getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP) && sd->state.pvp))))
	{
	...
	for(i = 0; i < MAX_INVENTORY; i++) {
		...
		if(sd->status.inventory[i].equip&~pc_equippoint(sd,i)) {
			...
			continue;
              ...
              if ( map->list[sd->bl.m].flag.noitem ) {
                       int j, slot;
                       struct item_data *i_data = itemdb_exists( sd->status.inventory[i].nameid );
                       ...
                       if ( !itemdb_isspecial( sd->status.inventory[i].card[0] ) ) {
                               for ( slot = 0; slot < MAX_SLOTS; slot++ ) {
                                       if ( sd->status.inventory[i].card[slot] ) {
                                               struct item_data *i_datac = itemdb_exists( sd->status.inventory[i].card[slot] );
